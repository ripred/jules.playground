...
def generate_story_graph(num_nodes, nodes_per_stage):
    """
    Generates a story graph with the specified number of nodes, partitioned into stages.
    """
    nodes = []
    ...
    for i in range(2, num_nodes + 1):
        ...
        # Find a random parent node within the current stage or previous stages.
        potential_parents = [n for n in nodes if len(n.get("choices", [])) < 3 and n.get("title") != "🏆 Game Over"]
        if not potential_parents:
            potential_parents = [n for n in nodes if n.get("title") != "🏆 Game Over"]
        ...
        elif i == num_nodes: # The very last node is the end of the game
            title = "🏆 Game Over"
